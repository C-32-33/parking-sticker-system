...
if not st.session_state.get("authenticated"):
    ...
    with col2:
        ...
        st.write("")
        email = st.text_input("📧 Email", placeholder="admin@society.com")
        password = st.text_input("🔑 Password", type="password")
        
        if st.button("Login", use_container_width=True, type="primary"):
            if email == "admin@society.com" and password == "admin123":
    ...
    with st.sidebar:
        st.markdown(f"""
            <div style='padding: 20px; text-align: center;'>
                ...
                <p style='margin: 5px 0; font-size: 13px;'>{st.session_state.user_role}</p>
            </div>
        """, unsafe_allow_html=True)
